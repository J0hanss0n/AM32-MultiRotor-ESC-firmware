...
void computeDshotDMA() {
  ...
  if((dshot_frametime < 1690) && (dshot_frametime > 1600)) {
    ...
    tocheck = (dpulse[0] << 10 | dpulse[1] << 9 | dpulse[2] << 8 | dpulse[3] << 7 |
...
void make_dshot_package() {
  ...
  gcrnumber = gcr_encode_table[(dshot_full_number >> 12) & 0xf] << 15 // first set of four digits
            | gcr_encode_table[(dshot_full_number >>  8) & 0xf] << 10 // 2nd set of 4 digits
            | gcr_encode_table[(dshot_full_number >>  4) & 0xf] << 5 //3rd set of four digits
            | gcr_encode_table[(dshot_full_number >>  0) & 0xf]; //last four digits
